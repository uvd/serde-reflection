...
/// Main configuration object for code-generation in php.
...
/// Shared state for the code generation of a php source file.
struct PhpEmitter<'a, T> {
    /// Writer.
...
impl<'a> CodeGenerator<'a> {
    /// Create a php code generator for the given config.
    pub fn new(config: &'a CodeGeneratorConfig) -> Self {
        if config.c_style_enums {
            panic!("php does not support generating c-style enums");
        }
    ...
    fn write_container_class(
        ...
        let mut file = std::fs::File::create(dir_path.join(name.to_string() + ".php"))?;
        let mut emitter = PhpEmitter {
    ...
    fn write_helper_class(
        ...
        let mut file = std::fs::File::create(dir_path.join("TraitHelpers.php"))?;
        let mut emitter = PhpEmitter {
            out: IndentedWriter::new(&mut file, IndentConfig::Space(4)),
            generator: self,
            current_namespace,
        ...
        T: Write,
{
    ...
    fn quote_type(&self, format: &Format) -> String {
        ...
        match format {
            ...
            Option(format) => format!("php.util.Optional<{}>", self.quote_type(format)),
            Seq(format) => format!("php.util.List<{}>", self.quote_type(format)),
            ...
            TupleArray { content, size } => format!(
                "php.util.@com.novi.serde.ArrayLen(length={}) List<{}>",
                size,
...
int count = 0;
for (php.util.Map.Entry<{}, {}> entry : value.entrySet()) {{
...
"#,
            ...
            TupleArray { content, size } => {
                write!(
                    ...
                    r#"
if (value.size() != {0}) {{
    throw new php.lang.IllegalArgumentException("Invalid length for fixed-size array: " + value.size() + " instead of "+ {0});
}}
for ({1} item : value) {{
    {2}
}}
...
if (!tag) {{
    return php.util.Optional.empty();
}} else {{
    return php.util.Optional.of({});
}}
"#,
...
php.util.List<{0}> obj = new php.util.ArrayList<{0}>((int) length);
...
"#,
            ...
            Map { key, value } => {
                write!(
                    ...
                    r#"
long length = deserializer.deserialize_len();
php.util.Map<{0}, {1}> obj = new php.util.HashMap<{0}, {1}>();
...
"#,
            ...
            TupleArray { content, size } => {
                write!(
                    self.out,
                    r#"
php.util.List<{0}> obj = new php.util.ArrayList<{0}>({1});
for (long i = 0; i < {1}; i++) {{
    obj.add({2});
}}
return obj;
"#,
    ...
    fn output_struct_or_variant_container(
        ...
        for field in fields {
            writeln!(
                self.out,
                "php.util.Objects.requireNonNull({0}, \"{0} must not be null\");",
                &field.name
            )?;
        }
        for field in fields {
...
{0} other = ({0}) obj;"#,
        ...
        for field in fields {
            writeln!(
                self.out,
                "if (!php.util.Objects.equals(this.{0}, other.{0})) {{ return false; }}",
                &field.name,
            )?;
        }
...
/// Installer for generated source files in php.
...
impl crate::SourceInstaller for Installer {
    ...
    fn install_serde_runtime(&self) -> std::result::Result<(), Self::Error> {
        self.install_runtime(
            include_directory!("runtime/php/serde"),
            "serde",
        )
    }
    ...
    fn install_bincode_runtime(&self) -> std::result::Result<(), Self::Error> {
        self.install_runtime(
            include_directory!("runtime/php/bincode"),
            "bincode",
        )
    }
    ...
    fn install_bcs_runtime(&self) -> std::result::Result<(), Self::Error> {
        self.install_runtime(
            include_directory!("runtime/php/bcs"),
            "bcs",
        )
    }
}
